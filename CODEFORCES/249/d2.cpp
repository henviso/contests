...
int main(){
	...
	REP(i, q){
		...
		if(x == 2){
			...
			for(int j = C; j<m && hh[j][R] >= j-C+1; j++){
			...
			if(a[R][C] == 1){
				for(int i = R; i<n && hv[i][C] >= i-R+1; i++) st_update_point(sv[i], hv[i], C, i-R);
				for(int j = C; j<m && hh[R][j] >= j-C+1; j++) st_update_point(sh[R], hh[R], j, j-C);
				a[R][C] = 0;
			}
			else{
				
				for(int i = R; i<n && hv[i][C] == i-R; i++) st_update_point(sv[i], hv[i], C, i-R+1);
				for(int j = C; j<m && hh[R][j] == j-C; j++) st_update_point(sh[R], hh[R], j, j-C+1);
				a[R][C] = 1;
			}
		}
	}
