...
int solve(int k){
    ...
    if(ans == -1){
        ans = 0;
        int v = e[k].second;
        int i = lower_bound(h[v].begin(), h[v].end(), k) - h[v].begin();
        int j = lower_bound(g[v].begin(), g[v].end(), ii(w[k]+1, -1)) - g[v].begin();
        int l = (i+1 < h[v].size())? w[h[v][i+1]]+1 : INF;
        
        if(i+1 < h[v].size()) ans = max(ans, solve(h[v][i+1]));
        REPP(J, j, g[v].size()){
            ii a = g[v][J];
            if(a.first >= l) break;
