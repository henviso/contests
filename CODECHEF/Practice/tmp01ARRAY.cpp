...
int main(){
	...
	ll totalSubs = 0LL;
	REP(i, n){
		cout << "i: " << i << " ACLCP[i] " << ACLCP[i] << endl;
	}
	REP(i, q){
		totalEq += equalSUBSTR(a[i], b[i]-1);
		int nlet = b[i]-a[i];
		cout << " INI " << a[i] << " FIM " << b[i] << endl;
		cout << " NUMERO DE LETRAS " << nlet << " SUBS " << (nlet*(nlet+1))/2 << " IGUAIS " << equalSUBSTR(a[i], b[i]-1) << endl;
		totalSubs += (nlet*(nlet+1))/2;
	}
	cout << "totalIguais " << totalEq << endl;
	cout << "totalStrings " << totalSubs << endl;
	cout << "RESP " << totalSubs - totalEq << endl;
