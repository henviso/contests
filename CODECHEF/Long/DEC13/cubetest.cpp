...
int sum2(int r1, int c1, int d1, int r2, int c2, int d2){
...
int main(){
	...
	while(t--){
		...
		REPP(i, 1, n+1) REPP(j, 1, n+1) REPP(k, 1, n+1){
			...
			int val = sum(1, 1, 1, i, j, k);
			if(val != s[i][j][k]) cout << " VAL EH " << val << " S EH " << s[i][j][k];
		...
		REPP(i, 1, n+1){
			...
			REPP(j, 1, n+1){
				...
				REPP(k, 1, n+1){
					...
					int soma2 = sum2(i, j, k, i+l, j+l, k+l);
					if(soma != soma2) cout << " EM " << i << " " << j << " " << k << " " << l << " SOMA EH " << soma << " SOMA2 EH " << soma2;
				}
			}
		}
	}
