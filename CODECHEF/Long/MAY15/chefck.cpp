...
void SlidingWindow(ll A[], int n, int K){
	REPP(i, 1, n+1){
		...
		if(i >= K) pd[i] = window.front().first;
	}
}

ll rmq(int L, int R){
	if(R < L) swap(R, L);
	return min(pd[R], pd[L+k-1]); 
}
...
int main(){
    ...
    REPP(x, 2, n+1){
		p = (p*t)%s;
		if(p <= r) A[x] = (A[x-1]*((a*A[x-1] + b)%m)+c)%m; 
		else A[x] = (A[x-1]*((d*A[x-1] + e)%m)+f)%m; 
	}
	SlidingWindow(A, ((int) n), ((int) k));
	ll L, R;
	REPP(i, 1, q+1){
		L1 = (La * L1 + Lc)%Lm;
		D1 = (Da * D1 + Dc)%Dm; 
